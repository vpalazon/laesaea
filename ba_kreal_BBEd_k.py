...
if __name__ == '__main__':
    ...
    lexperiment = [['kreal', 'kreal', 'BBEd', kcommand, 1],]
                ...
                for points in lpoints:
                    ...
                    for f in lfunctions:
                        ...
                        for experiment in lexperiment:
                            ...
                            fn_kt_wext = '%s_%s_%s_%s' % (fn, function, ext, method)

                            fn_kt_wext += '_%s_%s_k%02d_p%02d_%s' % (local_distance, subexp, k, step, hostname)

                            system_p("./tc -e %s -u %s -m %s -l %s -s %d -d %s -p %d -k %d -f %s_%s.%s > %s.kt" % (exp, subexp, method, local_distance, vector_size, data_type, step, k, fn, function, ext, fn_kt_wext))
