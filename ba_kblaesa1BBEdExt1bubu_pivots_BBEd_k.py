...
#   File: ba_kblaesa1BBEdExt1bubu_pivots_BBEd_k.py
...
hostname_pivots = gethostname()

INF = float('Infinity')

def extraePivots(fn):
    min_pivots = INF
    min_pivots_value = INF
    for line in file(fn):
        if line.startswith("#"):
            continue
        l = line.split()
        pivots = int(l[0])
        pivots_value = int(l[1])
        if pivots_value < min_pivots_value:
            min_pivots = pivots
            min_pivots_value = pivots_value
    return min_pivots, min_pivots_value
...
if __name__ == '__main__':
    ...
    lexperiment = [['klaesa', 'kblaesa1BBEdExt1bubu', 'BBEd', kcommand, 1],]
        ...
        for corpus in lcorpus:
            for sampling in lsampling:
                for points in lpoints:
                    ...
                    fn_pivots = fn.replace(corpus+samples, corpus_name+samples)
                    ...
                    for f in lfunctions:
                        ...
                        for experiment in lexperiment:
                            ...
                            fn_tt_bin_wext_pivots = fn_tt_bin_wext.replace(fn, fn_pivots)
                            fn_pivots = fn_tt_bin_wext_pivots + '_klaesa1_k%02d_p%02d_%s.pivots' % (1, step, hostname_pivots)

                            pivots, pivots_value = extraePivots(fn_pivots)

                            fn_kt_wext = fn_tt_bin_wext + '_%s_k%02d_p%02d_i%04d_%s' % (subexp, k, step, pivots, hostname)

                            system_p("./tc -e %s -u %s -m %s -l %s -s %d -d %s -p %d -k %d -f %s_%s.%s -t %s.tt -i %d > %s.kt" % (exp, subexp, method, local_distance, vector_size, data_type, step, k, fn, function, ext, fn_tt_bin_wext, pivots, fn_kt_wext))
